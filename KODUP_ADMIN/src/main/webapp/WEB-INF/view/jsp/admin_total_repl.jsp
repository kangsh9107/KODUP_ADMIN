...
<link defer rel="stylesheet" href="css/admin_total_repl.css">

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js" integrity="sha384-w76AqPfDkMBDXo30jS1Sgez6pr3x5MlQ1ZAGC+nuZB+EYdgRZgiwxhTBTkF7CXvN" crossorigin="anonymous"></script>
<script src="js/admin_total_repl.js"></script>


<title>admin_total_repl</title>
</head>
<body>
<main>
...
<!-- 검색 폼 -->
	<div style="padding-left:40px;">
	  <h3 style="font-weight: bold;">통합 댓글 관리</h3>
		  <hr><br>
	</div>
        ...
        <form class="frm_search" onsubmit="return false;">
          ...
          <div class="row g-3">
	          ...
	          <div class="col-sm-6 col-lg-6" style="margin-left:-50px;">
			     <input class="form-control form-control-sm findStr" type="search" name="findStr" value='${pVo.findStr}' aria-label=".form-control-sm example" placeholder="검색어를 입력하세요" style="height:38px;">
			   </div>
		...
		<c:forEach var='v' items="${list }" varStatus='status'>
			<li class='item'>
			    ...
			    <div onclick="view(${v.repl_sno }">
					...
					<span class='repl_doc'>
					   <c:choose>
					     <c:when test="${v.repl_delete eq 0}"> ${v.repl_doc }</c:when>
					     <c:when test="${v.repl_delete eq 1}">
					        <label style="color:red;">
					           ${v.repl_doc }(작성자 삭제)
					        </label>
					     </c:when>
					     <c:when test="${v.repl_delete eq 2}">
					        <label style="color:red;">
					           ${v.repl_doc }(삭제처리)
					        </label>
					     </c:when>
				 ...
				 <span class='delete_btn'>
				   <button type="button" class="btn btnDelete btn-danger btn-sm"
				           onclick="list_repl_delete(${v.repl_sno})">삭제</button>
				 </span>
				 <span class='restore_btn'>
				   <button type="button" class="btn btnRestore btn-success btn-sm"
				           onclick="list_repl_restore(${v.repl_sno})">복구</button>
				 </span>
			</li>
		</c:forEach>
